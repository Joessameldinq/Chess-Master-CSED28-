...
bool saveGame(FILE *fp, const Game *g)
{
    ...
    // Integers
    if (!writeInt(fp, g->moveCounter)) return false;
    if (!writeInt(fp, g->halfMoveClock)) return false;

    // Enums
    if (!writeEnum(fp, g->lastCapturedPiece)) return false;
...
bool loadGame(FILE *fp, Game *g)
{
    ...
    // Integers
    if (!readInt(fp, &g->moveCounter)) return false;
    if (!readInt(fp, &g->halfMoveClock)) return false;

    // Enums
    if (!readEnum(fp, (int*)&g->lastCapturedPiece)) return false;
